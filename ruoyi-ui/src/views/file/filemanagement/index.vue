    ...
    >
      ...
      <el-table-column label="制度名称" align="center" prop="regulationsTitle">
        <template slot-scope="scope">
          <a
            @click.prevent="previewFile(scope.row.pdfPath)"
            style="color: #6495ed"
            >{{ scope.row.regulationsTitle }}</a
          >
        </template>
      </el-table-column>
      <el-table-column
        label="专业分类"
        align="center"
        prop="classificationOfSpecialties"
      />
      <el-table-column
        label="制度编号"
        align="center"
      ...
      <el-table-column label="关联流程" align="center">
        <template slot-scope="scope">
          ...
          >
            <!-- popover：1、制度文件显示 -->
            <el-popover placement="bottom" title="绑定的流程" trigger="click">
      ...
      <el-table-column label="pdf下载" align="center" prop="pdfPath">
        <template v-slot:default="scope">
          <a
            ...
            @click.prevent="downloadFile(scope.row.pdfPath)"
            style="color: #6495ed"
            >点击下载</a
          >
        </template>
      ...
      <el-table-column label="word下载" align="center" prop="wordPath">
        <template v-slot:default="scope">
          <a
            ...
            :disabled="
              thisDept !== scope.row.mainResponsibleDepartment &&
              thisDept !== '研发' &&
              thisDept !== '企管' &&
              thisDept !== '总部'
            "
...
import {
  ...
  listAllFilemanagement,
  listHistoryFilemanagement,
  listAddFilemanagement,
...
} from "@/api/file/formfilemanagement"; //获取部门列表
  ...
  methods: {
    ...
    getList() {
      ...
      listFormfilemanagement(this.formQueryParams).then((response) => {
        ...
        const formTitlesToExclude = this.filemanagementList.map(
          (file) => file.formId
        );

        // 过滤formmanagementList，排除formTitlesToExclude中的记录
        this.formmanagementList = this.formmanagementList.filter(
          (form) => !formTitlesToExclude.includes(form.formTitle)
        );
      ...
      listHistoryFilemanagement(this.queryParams).then((response) => {
        ...
        console.log(
          "historyFilemanagementList=>",
          this.historyFilemanagementList
        );
      });
      //获取新增制度
    ...
    handleHistoryVersions(row) {
      ...
      this.$router.push(
        "/file/filemanagement/historyVersions/" + regulationsId
      );
    },
    /** 回收站路由跳转 */
    ...
    handleRevisionFrequency(row) {
      const regulationsId = row.regulationsId;
      this.$router.push(
        "/file/filemanagement/revisionFrequency/" + regulationsId
      );
    },
    /** 上传文件提交按钮 */
    uploadSubmitForm() {
        ...
        if (valid) {
          ...
          formTitles.forEach((formTitle) => {
            ...
            listFormfilemanagement(queryParams).then((response) => {
              ...
              const existingRegulations = resultForm[0].regulationId
                ? resultForm[0].regulationId.split(",")
                : [];
        ...
        if (valid) {
          if (this.form.regulationsId != null) {
            ...
            try {
              ...
              // 更新历史版本制度
              const lastFormResponse = await getFilemanagement(
                this.form.oldRegulationsId
              );
              ...
              const unbindPromises = formTitles.map((formTitle) => {
                ...
                return listFormfilemanagement(queryParams).then((response) => {
                  ...
                  const existingRegulations = resultForm[0].regulationId
                    ? resultForm[0].regulationId.split(",")
                    : [];
                  const updatedRegulations = existingRegulations.filter(
                    (regulation) => regulation !== lastForm.regulationsTitle
                  );
                  resultForm[0].regulationId = updatedRegulations.join(",");
                  console.log(
                    "Updated regulationId=>",
              ...
              // 将制度文件绑定到表单文件
              const bindPromises = this.form.formId
                .split(",")
                .map((formTitle) => {
                  const queryParams = { formTitle };
                  return listFormfilemanagement(queryParams).then(
                    (response) => {
                      const resultForm = response.rows;
                      ...
                      const existingRegulations = resultForm[0].regulationId
                        ...
                        : [];
                      if (
                        !existingRegulations.includes(
                          this.form.regulationsTitle
                        )
                      ) {
                        existingRegulations.push(this.form.regulationsTitle);
                        console.log(
    ...
    async handleDelete(row) {
      //2024.11.07 杜工傻逼 新增：如果当前制度绑定了流程需要自行解绑才能删除
      //2024.11.07---------------------------------------------------------------2024.11.07
      //projectNames
      //如果this.projectNames为null则提示“需要自行解绑才能删除”
      await this.handleProjectDetails(row);
      console.log("this.projectNames=>xhtxht", this.projectNames);
      if (this.projectNames.length != 0) {
        this.$modal.msgError("需要自行解绑流程中的绑定，才能删除/回收该文件！！！");
        return;
      } else {
        //2024.11.07---------------------------------------------------------------2024.11.07
        ...
        console.log("regulationsIds=>xhtxht", regulationsIds);
        ...
        const permissionCheckPromises = regulationsIds.map((id) => {
          return getFilemanagement(id).then((response) => {
            ...
            if (
              this.thisDept !== thisForm.mainResponsibleDepartment &&
              this.thisDept !== "研发" &&
              "企管" &&
              "总部"
            ) {
            ...
            return this.$modal.confirm("是否移入回收站？").then(() => {
              ...
              const updatePromises = forms.map((thisForm) => {
                ...
                const unbindPromises = formTitles.map((formTitle) => {
                  ...
                  return listFormfilemanagement(queryParams)
                    .then((response) => {
                      const resultForm = response.rows;
                      console.log("resultForm=>", resultForm);
                      const existingRegulations = resultForm[0].regulationId
                        ...
                        : [];
                      const updatedRegulations = existingRegulations.filter(
                        (regulation) => regulation !== thisForm.regulationsTitle
                      );
                      resultForm[0].regulationId = updatedRegulations.join(",");
                      console.log(
                        "Updated regulationId=>",
                ...
                return Promise.all(unbindPromises).then(() => {
                  ...
                  if (thisForm.oldRegulationsId != null) {
                    return getFilemanagement(thisForm.oldRegulationsId)
                      .then((response) => {
    ...
    pdfHandleUploadSuccess(res, file) {
      ...
      this.path = res.url;
      const uploadedFileName = uploadedFile.name.substring(
        0,
        uploadedFile.name.lastIndexOf(".")
      ); // 获取上传文件的文件名
      // 发起请求检查文件名是否存在于数据库中
      const isFileNameDuplicate = this.filemanagementList.some(
