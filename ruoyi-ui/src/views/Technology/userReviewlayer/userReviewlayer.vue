      ...
      <el-table-column label="用户审核操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            :disabled="scope.row.consultState !== '已通过'|| scope.row.userReviewState === '已通过'|| scope.row.userReviewState === '未通过'||scope.row.fileState === '协商通过,请更新用户审核文件'"
  ...
  methods: {
    ...
    getList() {
      ...
      listChangeorder(this.queryParams
        ...
        this.ChangeorderList = response.rows.filter(item => item.userReviewlayer ===  Number(newRegulationId) );
    ...
    handleDialogCancel() {
      ...
      this.currentRow.fileState = "审核不通过,等待更新文件并重新审核";
      saveToDatabase(this.currentRow);
      this.$message({
        type: 'error',
        message: '审核未通过',
