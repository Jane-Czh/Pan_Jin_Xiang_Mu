    ...
    methods: {
        async initData() {
            ...
            try {
                ...
                const res2 = await getCumulativeAverageIncomeData(timeDataBefore);
                this.dataBefore = res2.rows
                this.data.forEach(row => {
                    this.$set(row, 'cumulativeAverageIncomeBefore', '');
                });
                this.data.forEach((item, index) => {
                    ...
                    this.dataBefore.forEach((itemBefore, indexBefore) => {
                        ...
                        if (date1.getFullYear() - 1 === newDateBefore.getFullYear() && date1.getMonth() === newDateBefore.getMonth()) {
                            this.data[index].cumulativeAverageIncomeBefore = this.dataBefore[indexBefore].cumulativeAverageIncome
                        }
                    })
                });
            ...
            let series = [
                {
                    ...
                    data: this.data.map(item => item.cumulativeAverageIncome),
                },
                {
                    name: '同期收入',
                    type: 'bar',
