...
import { getMonthAmountInStockData } from '@/api/financial/data'
...
export default {
  ...
  methods: {
    async initData() {
      ...
      try {
        this.loading = true
        const res = await getMonthAmountInStockData(this.timeData);
        this.data = res.rows
        this.loading = false
        this.updateChart()
      } catch (error) {
    ...
    updateChart() {
      ...
      this.option = {
        ...
        series: [{
          ...
          data: this.data.map(item => item.Month_Amount_InStock),
        }]
      };
