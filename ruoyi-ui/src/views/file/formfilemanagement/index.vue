    ...
    >
      ...
      <el-table-column label="表单下载" align="center" prop="formPath">
        <template v-slot:default="scope">
          <a
            v-if="scope.row.formPath"
            @click.prevent="downloadFile(scope.row.formPath)"
            style="color: #6495ed"
            >点击下载</a
          >
        </template>
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      this.download(
        "file/formfilemanagement/export",
        {
          ...this.queryParams,
        },
    ...
    handleFileChange(file, formList) {
      ...
      // 将文件名填充到对应的输入框
      this.form.formName = uploadedFile.name.substring(
        0,
