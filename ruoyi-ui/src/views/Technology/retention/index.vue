...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="retentionList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
<!--      <el-table-column label="主键" align="center" prop="tfemId" />-->
      <el-table-column label="文件标题" align="center" prop="regulationsTitle" />
...
<!--      <el-table-column label="文件大小" align="center" prop="fileSize" />-->
      <el-table-column label="文件创建日期" align="center" prop="createDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.createDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="文件上传人" align="center" prop="uploadUsername" />
...
<script>
import { listRetention, getRetention, delRetention, addRetention, updateRetention } from "@/api/Technology/retention";
import {getUserProfile} from '@/api/system/user'
import {getDept} from '@/api/system/dept'
import {getToken} from "@/utils/auth"
import {word2Pdf} from "../../../api/Technology/retention";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除申报材料统计编号为"' + tfemIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('Technology/retention/export', {
        ...this.queryParams
    ...
    handleUploadSuccess(res, file) {
      ...
      // 发起请求检查文件名是否存在于数据库中
      const isFileNameDuplicate = this.retentionList.some(item => item.fileName === uploadedFileName);
