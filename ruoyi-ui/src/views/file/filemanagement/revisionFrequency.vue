...
<script>
  ...
  import {getRevisionCounts} from "../../../api/file/filemanagement";
  //获取流程相关的api
  ...
  export default {
    ...
    methods: {
      ...
      async initData() {
        const id = this.$route.params.regulationsId;
        this.timeData.regulationsId = id;
        ...
        try {
          ...
          const res = await getRevisionCounts(this.timeData);
          // console.log("res===>", res);
          let counts = 0;
          res.map((item) => {
            counts += item.times;
      ...
      updateChart() {
        ...
        this.option = {
          title: {
            text: "制度" + this.projectName + "修订频率统计（月）",
          },
          tooltip: {
            trigger: "axis",
