  ...
  data() {
    return {
      ...
      allIndex: [
        ...
        { id: 101, icon: 'el-icon-s-data', title: 'SAP管理指标', apiName: 'getManagementData', yDataName: 'Total_Sales_Revenue', dataName: '得分', content: 'SAP管理指标', },
        { id: 53, icon: 'el-icon-s-data', title: '日清日结指标', apiName: 'getFunctionalDeptOvertimeCostData', yDataName: 'Total_Sales_Revenue', dataName: '得分', content: '日清日结指标', },
      ],
      formData: {},
      rules: {},
    }
  ...
  methods: {
    ...
    toDetail(item) {
      if (item.id === 54 || item.id === 55 || item.id === 56 || item.id === 53 || item.id === 101) {
        this.$router.push(`/enterprise/indicators${item.id}`)
      } else {
        this.$router.push({
