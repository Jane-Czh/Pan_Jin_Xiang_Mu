      ...
      <el-table-column label="协商操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            :disabled="scope.row.examineState !== '已通过'|| scope.row.consultState === '已通过'|| scope.row.consultState === '未通过'|| scope.row.fileState === '审查通过,请更新协商文件'"
  ...
  methods: {
    ...
    getList() {
      ...
      listChangeorder(this.queryParams
        ...
        this.ChangeorderList = response.rows.filter(item => item.negotiationLayer ===  Number(newRegulationId) );
